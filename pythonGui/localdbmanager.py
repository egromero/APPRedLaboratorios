...
def recordsWriter(value):
	...
	if not os.path.isfile('./registrosOffline.csv'):
		...
		file.write('rfid,created_at,tipo\n')	

	with open('registrosOffline.csv', 'a') as file:
		text = value['rfid']+','+str(datetime.datetime.now())+','+value['tipo']+'\n'
		file.write(text)
...
def visitsRecordsWriter(value):
	...
	with open('registrosVisitasOffline.csv', 'a') as file:
		text = value['rut']+','+str(datetime.datetime.now())+','+value['motivo']+','+value['institucion']+'\n'
		file.write(text)
